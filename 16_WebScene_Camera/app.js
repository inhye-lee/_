...
function clearPOIHighlightAndSelection() {
  ...
  // ********** POINT OF CAM ACTIVATION: when pop up window is closed
  // (1) If selectedPOI was the only asset on MAP
  if (!track.tracking) {
    // DO NOTHING;
    return;
  }
  // (2) If the compass button is not active, but track is ON
  if (track.tracking && isCompassActive === false) {
    focusCamera(
      { latitude: useFilteredData ? filteredLat : lat, longitude: useFilteredData ? filteredLon : lon },
        true,
  ...
  // (3) If the compass button is active
  if (compassButton && isCompassActive === true) {
    focusCamera(
        { latitude: useFilteredData ? filteredLat : lat, longitude: useFilteredData ? filteredLon : lon },
        true,
        "smoothedHeading",      // useDynamicHeading
  ...
  webscene.view.whenLayerView(pointsLayer).then(function(layerView) {
    ...
    pointsLayer.queryFeatures(query).then(function(result) {
      ...
      if (!trackButton.tracking && selectedPOI) {
          ...
          // (2) When Track is ON, Compass is OFF: 
          // First, TURN track off (REMOVE LOADING TIME for USER)
          trackButton.stop(); // Turn off tracking after focusing on POI
          ...
          focusCamera(
            ...
            true,
            isCompassActive,
            false        // useMidpoint: show both user and POI 
          );
          
        } else if (trackButton.tracking && isCompassActive && selectedPOI) {
          // (3) When COMPASS IS ON, 
          // First, TURN track off, compass off (REMOVE LOADING TIME for USER)
          trackButton.stop();
          compassButton.style.display = 'none'; // Hide compass button when popup is closed
          isCompassActive = false; // Reset compass state
          compassOn_NoPOI = false;
          ...
          focusCamera(
            ...
            true,
            isCompassActive,
            false,        // useMidpoint: show both user and POI (show Relationship)
          );
        }
...
trackButton.addEventListener('click', () => {
  ...
  if (!selectedPOI && trackButton.tracking === false) {
    focusCamera(
      { latitude: useFilteredData ? filteredLat : lat, longitude: useFilteredData ? filteredLon : lon },
      true, "north", 1000, 300, false, isCompassActive, false 
    );
    return;
  }
  ...
  // 2. WHEN MY TRACK IS ACTIVATED
  // (3) POI selected, tracking turns ON
  if (selectedPOI && trackButton.tracking === true) {
    focusCamera(
      selectedPOI,
      true, "customHeading", 1000, 300, true, isCompassActive, true // useMidpoint: show both user and POI
    );
    return;
  }
  ...
  if (!selectedPOI && trackButton.tracking === true) {
    focusCamera(
      { latitude: useFilteredData ? filteredLat : lat, longitude: useFilteredData ? filteredLon : lon },
      true, "north", 1000, 300, true, isCompassActive, false
    );
    return;
  }
