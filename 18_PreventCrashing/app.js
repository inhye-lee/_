  ...
  require([
   ...
   //********************** Set up a Web Scene **********************//
    ...
    webscene.addEventListener("arcgisViewReadyChange", () => {
     ...
     // Place the goToOverride logic here with the SceneView Tracking Button
      if (trackButton && webscene && webscene.view) {
        trackButton.goToOverride = (view, goToParams) => {
          // Only called when tracking is ON
          if (isCompassActive === false) {
            if (selectedPOI && !POIselectedFromMap) { //Selected from AR
            focusCamera(
              selectedPOI,
              true,
              "recentHeading",
          ...
          } else if (selectedPOI && POIselectedFromMap) { //Selected from WebSCENE
            focusCamera(
              selectedPOI, // ZOOM IN TO THE SELECTED POI
              true,
              "recentHeading",
              1000,
              150,
            ...
            trackButton.stop(); // Stop tracking when a POI is selected from the map & Zoom IN
            POIselectedFromMap = false;
            return;
            // Stop tracking when a POI is selected from the map & Zoom IN
          } else if (!selectedPOI) {
            focusCamera(
        ...
        } // Compass is ACTIVE -> Taken care of in 
   ...
   if (smoothedTilting === null) {
    ...
    if (compassOn_NoPOI === true) {
      focusCamera(
        { latitude: useFilteredData ? filteredLat : lat, longitude: useFilteredData ? filteredLon : lon },
        true,
        "smoothedHeading", // useDynamicHeading
        1000,
        150,
        true,
        isCompassActive,
    ...
    if (compassOn_POI === true) {
      focusCamera(
        ...
        true,
        isCompassActive,
        true // useMidpoint: show both user and POI
      );
    }
  ...
  if (_headingInstruction = "smoothedHeading") {
   ...
   heading = (typeof smoothedHeading === "number" ? smoothedHeading : 0);
    ...
    // (2) Calculate Custom heading based on user location and selected POI
      ...
      heading = webscene.view.camera.heading; // Whatever heading that is in use;
    } else { // In all case
      heading = 0; 
    }
  ...
  webscene.view.whenLayerView(pointsLayer).then(function(layerView) {
    ...
    pointsLayer.queryFeatures(query).then(function(result) {
      if (result.features.length > 0) {
        ...
        // Focus camera on the selected POI

      // POINT OF CAM ACTIVATION ******* WHEN USER SELECTS A POI 
      if (!trackButton.tracking && selectedPOI) {
          // (1) When Track is OFF: focus directly on POI, no midpoint, no dynamic heading
          focusCamera(
            selectedPOI,
            true,
            "recentHeading",      // useDynamicHeading
            1000,
            150,
            false,
            isCompassActive,
        ...
        } else if (trackButton.tracking && !isCompassActive && selectedPOI) {
          ...
          // (3) When COMPASS IS ON - > Turn it OFF 
          // Fix FOR CONSTANT BOUNCE - User has to reactivate Compass once tapping on POI
          // BUT THE GENERAL HEADING IS STILL ALIVE (NOT CONSTANT)
          compassButton.style.display = 'none'; // Hide compass button when popup is closed
          isCompassActive = false; // Reset compass state
          compassOn_NoPOI = false;
          compassOn_POI = false; // Reset compass state
...
function updateCompassButtonState() {
  ...
  if (isInitialLocationAcquired) { 
    if (trackButton.tracking) {
      console.log("Track is enabled. State:", trackButton.state);
      if (isCompassActive) {
      ...
      console.log("Track is inactive. State:", trackButton.state);
    ...
    if (trackButton.tracking && trackButton.state !== "waiting") {
      console.log("Initial location acquired. Track state:", trackButton.state);
...
trackButton.addEventListener('click', () => {
  ...
  // _isCompassActive = false,
  // _useMidpoint = false
  // _useCustomHeading = false

  //******* */ POINT OF CAM ACTIVATION: when Track Button is clicked. ******* // 
  // 1. WHEN MY TRACK IS DEACTIVATED
  // (1) No POI selected, tracking TURNS OFF
  if (!selectedPOI && trackButton.tracking === false) {
    focusCamera(
      { latitude: useFilteredData ? filteredLat : lat, longitude: useFilteredData ? filteredLon : lon },
      true, "recentHeading", 1000, 150, false, isCompassActive, false 
    );
    return;
  }
  ...
  if (selectedPOI && trackButton.tracking === false) {
    focusCamera(
      selectedPOI, 
      true, "recentHeading", 1000, 150, false, isCompassActive, true
    );
    return;
  }

  // 2. WHEN MY TRACK IS ACTIVATED -> Taken care of in goToOverride()
  // (3) POI selected, tracking turns ON
  // if (selectedPOI && trackButton.tracking === true) {
  // }

  // // (4) No POI selected, tracking turns ON
  // if (!selectedPOI && trackButton.tracking === true) {
  // }
...
document.getElementById('compassButton').addEventListener('click', () => {
  ...
  if (!selectedPOI && trackButton.tracking === true && isCompassActive === true) {
  ...
  //********/ Compass turned OFF
  // (3) POI selected, tracking OFF
  if (selectedPOI && isCompassActive === false && trackButton.tracking === false) {
    compassOn_NoPOI = false;
    compassOn_POI = false;
    return;
  }

  // (4) while POI selected, Compass turned OFF, tracking still ON
  if (isCompassActive === false && selectedPOI &&  trackButton.tracking === true) {
    // focusCamera(
    //   selectedPOI,
    //   true,
    //   "customHeading", // useDynamicHeading
    //   1000,
    //   150,
    //   false,
    //   isCompassActive,
  ...
  // (5) No POI selected, Compass turned OFF, tracking still ON
  if (isCompassActive === false && !selectedPOI && trackButton.tracking === true) {
    // focusCamera(
    //   { latitude: useFilteredData ? filteredLat : lat, longitude: useFilteredData ? filteredLon : lon },
    //   true,
