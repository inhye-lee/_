    ...
    webscene.addEventListener("arcgisViewReadyChange", async () => {
      ...
      //******* CUSTOM TRACK BUTTON BEHAVIOR ********//
       trackButton.addEventListener('click', () => {
        ...
        trackButton.textContent = isTracking ? "Tracking On" : "Tracking Off";
        trackButton.style.backgroundColor = isTracking ? "#28a745" : "#d3d3d3"; // Update color
  ...
  if (event.alpha !== null) {
   ...
   if (smoothedTilting === null) {
    ...
    if (compassOn_NoPOI === true) {
      ...
      const screenY = webscene.view.height * 0.85;
  ...
  webscene.view.whenLayerView(pointsLayer).then(function(layerView) {
    ...
    pointsLayer.queryFeatures(query).then(function(result) {
      ...
      if (!isTracking && selectedPOI) { // !trackButton.tracking && selectedPOI
          ...
          compassButton.style.backgroundColor = '#d3d3d3'; // Set color to grey (Compass Off)
  ...
  if (isInitialLocationAcquired) { 
    if (isTracking) {
      ...
      if (isCompassActive) {
        ...
        updateCompassButton(true, false, 'Compass Off', '1', '#d3d3d3'); // Show and deactivate the compass button
      }     
    } else {
      console.log("Track is inactive. State: ", isTracking);
      updateCompassButton(false, false, 'Compass Off', '0.75', '#d3d3d3'); // Hide and deactivate the compass button
      deactivateCompass();
    }
...
function moveCompassToTrackPosition() {
  animateCompassToPosition(30); // Animate to bottom: 30px
  compassButton.style.left = "18px";
}

// Utility to restore compass button to its default position with animation
function restoreCompassPosition() {
  animateCompassToPosition(70); // Animate to bottom: 70px
  compassButton.style.left = "18px";
}
  ...
  if (expand) {
    const observer = new MutationObserver(() => {
      ...
      if (isLegendExpanded) {
        ...
        // if (trackButton) trackButton.style.display = ''; // Show track button when legend is collapsed
      }
    });
