...
AFRAME.registerComponent('raycaster-handler', {
  ...
  AFRAME.registerComponent('toggle-title', {
    ...
    tick: function () {
      ...
      // (NOT WORKING)
      // // --- Manual look-at logic for textParent --- (BREAKS THE POI TEXT and the bg)
      // if (this.textParent && camera) {
      //   const cameraWorldPos = new THREE.Vector3();
      //   camera.getWorldPosition(cameraWorldPos);

      //   const textWorldPos = new THREE.Vector3();
      //   this.textParent.object3D.getWorldPosition(textWorldPos);

      //   const dir = new THREE.Vector3().subVectors(cameraWorldPos, textWorldPos).normalize();

      //   const targetQuat = new THREE.Quaternion();
      //   targetQuat.setFromUnitVectors(new THREE.Vector3(0, 0, 1), dir);

      //   this.textParent.object3D.quaternion.copy(targetQuat);

      //   // If mirrored, uncomment:
      //   // this.textParent.object3D.rotateY(Math.PI);
      // }
      // // --- End manual look-at logic ---
    ...
    pinTitle: function () {
      ...
      if (this.textParent) {
        ...
        if (!this.halo) {
          ...
          halo.setAttribute('material', 'color: white; metalness: 0.05; roughness: 1'); // Ensure it's visible from both sides
  ...
  AFRAME.registerComponent('show-popup', {
    ...
    openPopup: function () {
      ...
      if (!popup) {
        ...
        popup.style.top = 'auto'; // Allow positioning at the bottom
  ...
  // * Fix for Camera Rig Rotation (Not working)
//   AFRAME.registerComponent('poi-lookat-camera', {
//   tick: function () {
//     // Get the camera object3D
//     const camera = document.querySelector('[gps-camera]').object3D;
//     if (!camera) return;

//     // Get this POI's text entity (adjust selector as needed)
//     const textParent = this.el.querySelector('.text-parent');
//     if (!textParent) return;

//     // Get world position of camera and text
//     const cameraWorldPos = new THREE.Vector3();
//     camera.getWorldPosition(cameraWorldPos);

//     const textWorldPos = new THREE.Vector3();
//     textParent.object3D.getWorldPosition(textWorldPos);

//     // Compute direction from text to camera
//     const dir = new THREE.Vector3().subVectors(cameraWorldPos, textWorldPos).normalize();

//     // Calculate the quaternion that rotates the text to face the camera
//     const targetQuat = new THREE.Quaternion();
//     // Default forward vector is (0,0,1) in A-Frame
//     targetQuat.setFromUnitVectors(new THREE.Vector3(0, 0, 1), dir);

//     // Apply the quaternion to the textParent
//     textParent.object3D.quaternion.copy(targetQuat);
//   }
// });


// Add this at the very end in aframe-component.js 
