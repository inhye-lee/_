...
let selectedPOI = null; // To keep track of the selected POI. There can be only one
let previousPOI = null; // 
...
function showUserLocationMarker(lat, lon) {
  ...
  let marker = webscene.view.graphics.find(g => g.attributes && g.attributes.type === 'user-location');
  if (marker) webscene.view.graphics.remove(marker);
  ...
  webscene.view.graphics.add(graphic);
}
...
document.addEventListener('ar-poi-selected', function(e) {
  ...
  const track = document.getElementById("track");
  console.log("AR POI selected with ID:", poiId);
  webscene.view.whenLayerView(pointsLayer).then(function(layerView) {
    ...
    pointsLayer.queryFeatures(query).then(function(result) {
      if (result.features.length > 0) {
        ...
        handleCameraInWebScene(); // Run it here!

        if (compassButton) {
          updateCompassButton(false, false, 'Compass is Off', '0.75', '#979898'); // Hide and deactivate the compass button
          deactivateCompass();
        }
      }
    });
...
document.addEventListener('web-popup-closed', function(e) {
  selectedPOI = null;
});

// ********************** Handle Camera Logic in WebScene **********************//
function handleCameraInWebScene() {
    // WebSCENE Camera logic for all cases are handled here
    const track = document.getElementById("track");
    let tracking = track.tracking; // Get the current tracking state

   //********* */ USER PATHS: From a POI or the TRACK Component
   // (1) NO TRACK 
    if (!tracking) { // Tracking is NOT active
      if (selectedPOI) { // Then a POI is selected
        focusCameraOnPOI(selectedPOI, true, 0); // poi, animate, heading
      } 
    } 

    // (2) selectedPOI Changes
    if (selectedPOI && (
        !previousPOI ||
        selectedPOI.latitude !== previousPOI.latitude ||
        selectedPOI.longitude !== previousPOI.longitude
      )) {
      focusCameraOnPOI(selectedPOI, true, 0); // poi, animate, heading
      previousPOI = { ...selectedPOI }; // Store a copy of the current POI
    }

   // (3) NO TRACK, selected POI Then Tracking
    if (!tracking && selectedPOI) {
      if (tracking) { // Then Tap on Tracking
        // focusCameraOnPOI(selectedPOI, true, 0);
        focusCameraOnPOIWithTrack(selectedPOI, true, 0); //  poi, animate, heading
      }
    } 
    
    // (4) TRACK, NO COMPASS ->
    if (tracking && !isCompassActive) { // Tracking is active && Compass OFF
      if (selectedPOI) { // Then a POI is selected
        focusCameraOnPOI(selectedPOI, true, 0); // poi, animate, heading
        track.stop(); // Stop Tracking -> Stop Compass
      } else { // POI NOT yet selected
        focusCameraOnUser(true, false, 1000, 300); // animate, useDynamicHeading, offsetHeight, offsetBehind
      } 
    }
    
    // (5) COMPASS -> 
    if (isCompassActive && selectedPOI) {
      focusCameraOnPOI(selectedPOI, true, 0); // poi, animate, heading
      track.stop(); // Stop tracking -> Stop Compass
    } 
}

// ********************** Focus WebScene Camera on POI **********************//
function focusCameraOnPOI(poi, animate = true, heading = 0) {
  if (!poi) return;
  selectedPOI = poi;

  const offset = getCameraOffsetPosition(poi.latitude, poi.longitude, heading, 1000, 300);
  ...
  webscene.view.goTo({
    ...
    tilt: offset.tilt,
    heading: heading
  }, { animate: animate }).catch((error) => {
    if (error.name !== "AbortError") {
      console.error("Error focusing on POI:", error);
    }
  });
}

function focusCameraOnPOIWithTrack(poi, animate = true, heading = 0) {
  if (!poi) return;
  selectedPOI = poi;
  ...
  // Calculate the distance between user and POI
  const poiDistance = calculateDistance(userLat, userLon, poi.latitude, poi.longitude);

  // Calculate the midpoint between user and POI
  const midLat = (userLat + poi.latitude) / 2;
  const midLon = (userLon + poi.longitude) / 2;

  // Set camera offset to be 1.5x the distance between user and POI (so both are visible)
  // Clamp to a minimum and maximum for usability
  const distanceMeters = Math.max(500, Math.min(poiDistance * 1.5, 5000));
  const altitudeOffset = Math.max(200, Math.min(poiDistance * 0.8, 3000));

  // Use the provided heading (static if compass is off, dynamic if on)
  const offset = getCameraOffsetPosition(midLat, midLon, heading, distanceMeters, altitudeOffset);
  ...
  webscene.view.goTo({
    ...
    if (error.name !== "AbortError") {
      console.error("Error focusing on POI with user location:", error);
    }
  });
}

// ********************** Get Camera Offset Position **********************//
function focusCameraOnUser(animate = true, useDynamicHeading = false, offsetHeight = 1000, offsetBehind = 300) {
  const gpsLat = useFilteredData ? filteredLat : lat;
  const gpsLon = useFilteredData ? filteredLon : lon;
  const heading = useDynamicHeading ? smoothedHeading : 0;
  const offset = getCameraOffsetPosition(gpsLat, gpsLon, heading, offsetHeight, offsetBehind);
  ...
  webscene.view.goTo({
    ...
    if (error.name !== "AbortError") {
      console.error("Error focusing camera on user:", error);
    }
  });
}
...
function updateOrientation(event) {
  ...
  if (event.alpha !== null) {
   ...
   if (smoothedTilting === null) {
    ...
    // (5) COMPASS - We handle this HERE to receive the contantly updated Heading when no interaction w/POI   
    if (isCompassActive && !selectedPOI) { // 
      // Tracking, compass on, no POI: follow user with compass orientation
      // * Fix - For unknown reason, a single logic not working for all platforms so repeating
      switch (platform) {
        case "iOS":
          console.log("Platform: iOS");
          focusCameraOnUser(true, true, 1000, 300); // animate, useDynamicHeading, offsetHeight, offsetBehind
          break;
        case "Android":
          console.log("Platform: Android");
          focusCameraOnUser(true, true, 1000, 300);
          break;
        default:
...
function updateCompassButtonState() {
  ...
  // Hide compassButton if tracking is not active
  if (!track.tracking || track.state === "disabled" || track.state === "stopped") {
  ...
  if (isInitialLocationAcquired) { 
    if (track.tracking) {
      ...
      if (isCompassActive) {
        toggleViewButton.style.display = 'none'; // Hide when compass is ON
        updateCompassButton(true, true, 'Compass is On', '1', '#28a745'); // Show and activate the compass button
      } else {
        toggleViewButton.style.display = 'block'; // Show when compass is Off
        updateCompassButton(true, false, 'Compass is Off', '1', '#979898'); // Show and deactivate the compass button
      }     
    } else {
      toggleViewButton.style.display = 'block'; // Show when tracking is on
...
// Called after initial calibration is complete
    ...
    headingYesBtn.addEventListener('click', () => {
      ...
      // document.getElementById('toggleView').style.display = 'block'; // Show the toggle view button
    });
...
// *** Event Listeners for TRACK BUTTON ***
document.getElementById('track').addEventListener('click', function() {
  // If a POI is already selected when the track button is pressed
  const track = document.getElementById('track');
  // When the track is being used already;
  if (track.tracking && !selectedPOI) {
    // If no POI is selected, focus on the user
    focusCameraOnUser(true, false, 1000, 300); // animate, useDynamicHeading, offsetHeight, offsetBehind
  }

  if (track.tracking && selectedPOI) {
    // Example: Focus camera to show both user and POI
    focusCameraOnPOIWithTrack(selectedPOI, true, isCompassActive ? smoothedHeading : 0);
    // Optionally, clear the POI selection after focusing
    selectedPOI = null;
  }
});
