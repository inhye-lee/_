  ...
  if (modal) {
    ...
    const interval = setInterval(() => {
      ...
      if (secondsLeft > 0) {
        ...
        isCompassCalibrated = true; // Mark calibration as complete
        // if (isCompassCalibrated) {
        //   updateDisplay();
        // }
...
function updateOrientationIndicator() {
  ...
  if (window.lastOrientationEvent) {
    const reliable = isCompassReliable(window.lastOrientationEvent);
    reliabilityMsg.textContent = reliable
      ? " Compass: Reliable"
      : " Compass: Unreliable";
    reliabilityMsg.style.color = reliable ? "#28a745" : "#d13434";
  } else {
    reliabilityMsg.textContent = " Compass: Checking...";
    reliabilityMsg.style.color = "#888";
  }
}
